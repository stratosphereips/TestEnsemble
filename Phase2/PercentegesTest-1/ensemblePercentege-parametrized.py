       ...
       for dst in list(set(df[df.SrcAddr == src].DstAddr)):
           for proto in list(set(df[(df.SrcAddr == src) & (df.DstAddr == dst)].Proto)):
               for state_orig in list(set(df[(df.SrcAddr == src) & (df.DstAddr == dst) & (df.Proto == proto)].State)):
                   ...
                   if(((d2[key]['TCPEstablishedPercentage']>TCPEP)&(d2[key]['cantTCPE']>countTCPE))|((d2[key]['TCPNotEstablishedPercentage']>TCPNEP)&(d2[key]['cantTCPNE']>countTCPNE))|((d2[key]['UDPEstablishedPercentage']>UDPEP)&(d2[key]['cantUDPE']>countUDPE))|((d2[key]['UDPNotEstablishedPercentage']>UDPNEP)&(d2[key]['cantUDPNE']>countUDPNE))):
