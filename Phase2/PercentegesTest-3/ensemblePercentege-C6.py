    ...
    for dst in list(set(df[df.SrcAddr == src].DstAddr)):
        for proto in list(set(df[(df.SrcAddr == src) & (df.DstAddr == dst)].Proto)):
            for state_orig in list(set(df[(df.SrcAddr == src) & (df.DstAddr == dst) & (df.Proto == proto)].State)):
                ...
                if (((d2[key]['TCPEstablishedPercentage']==0)|(d2[key]['cantTCPE']<4))&((d2[key]['TCPNotEstablishedPercentage']==0)|(d2[key]['cantTCPNE']<4))&((d2[key]['UDPEstablishedPercentage']==0)|(d2[key]['cantUDPE']<4))&((d2[key]['UDPNotEstablishedPercentage']==0)|(d2[key]['cantUDPNE']<4))):
