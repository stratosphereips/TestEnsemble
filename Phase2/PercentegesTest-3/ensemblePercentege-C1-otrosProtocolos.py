...
def process_file (filename):
   ...
   dataset = pd.read_csv(archivo, sep=',', parse_dates=['StartTime'])
   ...
   dataset.Label = dataset.Label.str.replace(r'(^.*Normal.*$)', 'Normal')
   #agregué esta linea para este dataset
...
nombrearchivo='2018-05-03_win11.binetflow'
...
for t in df.itertuples():
    ...
    if t.DstAddr not in d[t.SrcAddr].keys():
        ...
        d2[t.SrcAddr+"-"+t.DstAddr] = {'SrcAddr': t.SrcAddr, 'DstAddr': t.DstAddr, 'TCPEstablishedPercentage': 0, 'TCPNotEstablishedPercentage': 0, 'UDPEstablishedPercentage': 0, 'UDPNotEstablishedPercentage': 0,'cantTCPE': 0,'cantTCPNE': 0, 'cantUDPE': 0, 'cantUDPNE': 0, 'OthersPercentage':0, 'cantOthers':0, 'originalLabel': ''}
...
infectadas=set(['192.168.1.121'])
limpias=set(['192.168.1.2'])
    ...
    for dst in list(set(df[df.SrcAddr == src].DstAddr)):
        for proto in list(set(df[(df.SrcAddr == src) & (df.DstAddr == dst)].Proto)):
            for state_orig in list(set(df[(df.SrcAddr == src) & (df.DstAddr == dst) & (df.Proto == proto)].State)):
                ...
                if(proto=='tcp'):
                   ...
                   if(proto=='udp'):
                       if (state=='Established'):
                           ...
                           d2[key]['OthersPercentage']=pproto
                           d2[key]['cantOthers']=d[src][dst][proto]['total3']
                ...
                if ((d2[key]['TCPEstablishedPercentage']==0)&(d2[key]['TCPNotEstablishedPercentage']==0)&(d2[key]['UDPEstablishedPercentage']==0)&(d2[key]['UDPNotEstablishedPercentage']==0)&(d2[key]['OthersPercentage']==0)):
...
df2['OthersPercentage'] = [value['OthersPercentage'] for value in d2.values()]
...
df2['cantOthers'] = [value['cantOthers'] for value in d2.values()]
