...
clf6 = LogisticRegression(C=100, random_state=SEED)
clf7 = MLPClassifier((80, 10), early_stopping=False, random_state=SEED)
...
print("Voting sin peso con LR, RF y NB")

#Cambiando soft por hard es Majority voting
#SI es soft tiene en cuenta las probabilidades de cada clasificador
eclf = VotingClassifier(estimators=[(l1, clf1), (l2, clf2), (l3, clf3)], voting='soft')

for clf, label in zip([clf1, clf2, clf3, eclf], [l1, l2, l3, l8]):
     scores = cross_val_score(clf, X_test_mixed_std, y_test_mixed, cv=5, scoring='accuracy')
     print("Accuracy: %0.10f (+/- %0.10f) [%s]" % (scores.mean(), scores.std(), label))


print("Voting cambiando RF por SVC")

eclf = VotingClassifier(estimators=[(l1, clf1), (l2, clf5), (l3, clf3)], voting='soft')
